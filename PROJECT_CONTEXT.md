...
| Commit | Decisión |
|---|---|
| 2026-06-06 v5 | fix(nexa): Sincronizar estilos carrusel con render real — Bug: todos los estilos caían en un único template genérico (solo diferenciaban bg-color y badge pequeño). Fix: 5 templates HTML estructuralmente distintos en _render_carrusel() por estilo_id: LISTA_NUMERADA (layout magazine: número gigante izquierda + contenido derecha separado por borde), MITOS_REALIDAD (veredicto centrado: ❌/✅ 50px + badge MITO/REALIDAD dominante + dots de progreso), CASO_EXITO (timeline de fases superior: CLIENTE/DESAFÍO/PROCESO/RESULTADO con colores propios + tarjeta activa), TUTORIAL_PASOS (barra de progreso + número de paso 46px semitransparente + porcentaje completado), PROBLEMA_SOLUCIÓN (default con _slide_body por tipo). Variables pre-computadas (tit/sub_p/tpl_marker) para compatibilidad Python 3.11. Marcadores debug visibles "TEMPLATE: estilo_id" en cada slide no-portada. Validado: 5 templates × todos distintos, marcadores presentes, django check 0 issues. |
| 2026-06-06 v4 | feat(nexa): Tipografía adaptativa — helper _afs(text, fmt) en agente_diseno_instagram.py: font-size escalado por longitud (post/slide/story/reel). CSS overflow-wrap + word-break + line-clamp en todos los formatos. .nxar-post-frame h2 selector cubre renders POST inline. Reglas 1-7 cubiertas: max-width, font-size adaptativo, overflow-wrap, line-clamp, contenedores con overflow:hidden. |
